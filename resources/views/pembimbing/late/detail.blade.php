...
@section('content')
    ...
    <div class="title" style="margin: 4px 15px; ">
        ...
        <a href="/home">Dashboard / </a><a href="{{ route('ps.late.rekap') }}"> Data Keterlambatan / </a><a> Detail Data
            Keterlambatan</a>
    </div><br><br>

    <div class="name" style="margin: 4px 15px;">
        <h3 style="color: #012970;"> {{ $student['name'] }} </h3>
        <h6 style="color: #8b9ec1;"> | {{ $student['nis'] }} | {{ $student['rombel']['rombel'] }} |
            {{ $student['rayon']['rayon'] }} </h6>
    </div>
        ...
        @foreach ($late as $item)
            <div class="card m-2 shadow p-2 bg-body rounded row flex-col">
                <div class="card-body p-3">
                    <h5 class="card-title shadow-md" style="color: #012970; text-align:center;">Keterlambatan ke-
                        {{ $no++ }}</h5> <br>
                    <div>
                        ...
                        <img src="{{ asset('post_bukti/' . $item->bukti) }}"
                            class="img-preview img-fluid mb-3  col-sm-15  d-block ">
                    </div>
                </div>
            </div>
        @endforeach
